...
assert(evidence && evidence.status === 'fallback_used' && evidence.fallback_reason === 'config_blocked', 'config_blocked fallback evidence');
...
save('stage3a-cost-report.json', { ...baseRecord(budgetCreated.work, budgetCreated.generation_job, buildGenerationManifest(budgetCreated.work.id), budgetEvidence), cost_estimate_usd: budgetEvidence.cost_estimate_usd, max_budget_usd: 0.001, budget_blocked: true, live_charge: false, hard_stop_on_budget_exceeded: true });
save('stage3a-secret-scan.json', { ...base, passed: scan.passed, hits: scan.hits, no_secrets: scan.passed });
save('stage3a-qc-summary.md', `# Stage 3A QC Summary\n\n- timestamp: ${now()}\n- repo: ${repo}\n- branch: ${branch}\n- commit: ${commit}\n- work_id: ${work.id}\n- generation_job_id: ${job.id}\n- provider: fal.ai\n- config_blocked path: PASS\n- provider success path: SKIPPED_NO_OWNER_ENABLE\n- timeout/failed path: fallback evidence covered by config/budget fixtures\n- cost cap: PASS\n- secret scan: PASS\n- production deploy: NO\n- PR merge: NO\n- live payment: NO\n- formal DB: NO\n`);
console.log(JSON.stringify({ status: 'PASS', config_blocked: 'PASS', provider_success: 'SKIPPED_NO_OWNER_ENABLE', cost_cap: 'PASS', secret_scan: 'PASS', evidence_dir: evidenceDir }));
